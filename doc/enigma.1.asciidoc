...
enigma - Binary space partitioning window manager

Synopsis
--------

*enigma* [*-h*|*-v*|*-c* 'CONFIG_PATH']

*enigmac* 'DOMAIN' ['SELECTOR'] 'COMMANDS'

*enigmac* 'COMMAND' ['OPTIONS'] ['ARGUMENTS']

Description
-----------

*enigma* is a tiling window manager that represents windows as the leaves of a full binary tree.

It is controlled and configured via *enigmac*.
...
*-c*, *--count* 'COUNT'::
	Stop the corresponding *enigmac* process after having received 'COUNT' events.
...
If the server can't handle a message, *enigmac* will return with a non-zero exit code.
...
Please note that *enigma* initializes monitors before it reads messages on its socket, therefore the initial monitor events can't be received.
...
'ENIGMA_SOCKET'::
	The path of the socket used for the communication between *enigmac* and *enigma*. If it isn't defined, then the following path is used: '/tmp/enigma<host_name>_<display_number>_<screen_number>-socket'.
...
Eternal Acwars <eternalacwars at gmail.com>
